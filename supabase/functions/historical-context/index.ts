...
serve(async (req) => {
  ...
  try {
    ...
    // Build context-aware prompt for historical information
    ...
    if (migrationPattern) {
      prompt += `. Focus on migration patterns and explain why people might have moved during this time period (e.g., Dust Bowl, Gold Rush, Industrial Revolution, war, etc.).`;
    }
    ...
    const historicalContext = data.choices?.[0]?.message?.content || "No historical context available.";

    return new Response(
      JSON.stringify({ context: historicalContext }),
